...
library(tidyverse)
read.csv(here("R", "data", "AirBase_v8_stations.csv"), sep = "\t", stringsAsFactors = FALSE) %>% 
  as_tibble  %>% 
...
st_as_stars(matrix(0, length(dx), length(dy))) %>%
  ...
  st_set_crs(crs) -> grd
i = st_intersects(grd, cy.sf)
grd[[1]][lengths(i)==0] = NA
grd

v <- gstat::variogram(NO2~1, no2.sf)
plot(v, plot.numbers = TRUE)
v.m = gstat::fit.variogram(v, gstat::vgm(1, "Exp", 50000, 1))
